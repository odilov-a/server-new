...
exports.createWithFolder = async (req, res) => {
  try {
    ...
    const test = await Test.create({ name: lines[0], subject: req.body.subject });

    const savedQuestions = await Question.insertMany(
      questions.map(q => ({ ...q, test: test._id }))
    );

    res.json({ message: `${savedQuestions.length} ta savol saqlandi`, test });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
exports.create = async (req, res) => {
  try {
    ...
    await Question.insertMany(req.body.questions.map(q => ({ ...q, test: test._id })));
    return res.json({ message: "Successfully created", test });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
}
